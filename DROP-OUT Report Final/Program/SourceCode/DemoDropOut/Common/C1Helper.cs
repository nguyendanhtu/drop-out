...
namespace DemoDropOut.Common
{
    public class C1Helper
    {
        public static void LoadDataTableToC1Grid(C1FlexGrid ip_c1Grid, DataTable ip_table)
        {
            ...
            ip_c1Grid.DataSource = ip_table;
            for (int i = 0; i < ip_table.Columns.Count; i++)
            {
                var v_col = ip_c1Grid.Cols.Fixed + i;
                ip_c1Grid.Cols[v_col].Caption = ip_table.Columns[i].Caption;
                ip_c1Grid.Cols[v_col].UserData = ip_table.Columns[i].ExtendedProperties["Details"];
            }
            #region Old Version
